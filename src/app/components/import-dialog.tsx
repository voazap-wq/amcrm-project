...
interface ImportDialogProps {
  ...
  onImport: (data: ReceivingFormValues, applyMarkupRules: boolean) => Promise<{ createdCount: number, updatedCount: number, newDocId: string | null }>;
...
export function ImportDialog({ children, suppliers, warehouseCells, markupRules, productCategories, onImport, lastImportId, onDeleteImport }: ImportDialogProps) {
  ...
  const [applyMarkup, setApplyMarkup] = React.useState(false);
  ...
  const handleImport = async () => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        const { createdCount, updatedCount, newDocId } = await onImport({ products: productsToImport }, applyMarkup);
