...
import type { CustomerOrder, Car, StoreDetails } from "@/lib/types";
import {
  Table,
  TableBody,
  TableCell,
...
interface SalesReceiptNoArticleProps {
  order: CustomerOrder;
  car?: Car | null;
  storeDetails: Partial<StoreDetails>;
}

// Function to convert number to words (remains the same)
...
export const SalesReceiptNoArticle = React.forwardRef<HTMLDivElement, SalesReceiptNoArticleProps>(
    ...
    return (
      <div ref={ref} className="p-8 bg-white text-black font-sans text-xs w-full max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-2 mb-4">
            ...
            <div className="col-span-2"><span className="font-bold">Основание:</span> {basis}</div>
          ...
          <TableBody>
            {activeItems.map((item, index) => (
              <TableRow key={item.id} className="border-black">
                ...
                <TableCell className="border border-black p-1 text-xs">{item.name} {item.manufacturer && `(${item.manufacturer})`}</TableCell>
...
SalesReceiptNoArticle.displayName = "SalesReceiptNoArticle";
